...
public class PriorityQueue {
	...
	public int removeMax() {
		...
		while(parentIndex < data.size()) {
			...
			if(leftChildIndex < data.size()) {
				if(data.get(leftChildIndex) > data.get(maxIndex)) {
					maxIndex = leftChildIndex;
				}
			}
			if(rightChildIndex < data.size()) {
				if(data.get(rightChildIndex) > data.get(maxIndex)) {
			...
			if(maxIndex == parentIndex) {
				...
				int temp = data.get(maxIndex);
				data.set(maxIndex, data.get(parentIndex));
				data.set(parentIndex, temp);
