...
// self practice function	
	public static TreeNode<Integer> takeInputLevelWise2() throws QueueEmptyException {
		QueueUsingLL<TreeNode<Integer>> pendingQueue = new QueueUsingLL<TreeNode<Integer>>();
		Scanner s = new Scanner(System.in);
		int data = s.nextInt();
		...
		while(pendingQueue!=null){
			TreeNode<Integer> current = pendingQueue.dequeue();
			System.out.println("Enter No. of Children of node " + current.data + ":- " );
			int numOfChildren = s.nextInt();
			for(int i = 0; i<numOfChildren; i++){
				System.out.println("Enter value of " + i + "th children :- ");
				int dataValue = s.nextInt();
				TreeNode<Integer> child = new TreeNode<Integer>(dataValue);
				current.children.add(child);
